...
func (e *events) Fire(event Event) {
	e.announcements <- event
}

func (e *events) Signal(tag string) {
	e.Fire(Event{Tag: tag, Data: nil})
}
...
// closing.
func EventListen(prefix string) chan Event {
	return globalEvents.Listen(prefix)
}

// Announce will register the global occurance of the given Event e, and pass
// it along for dispatch to interested listeners.
func EventFire(event Event, delay time.Duration) {
	time.AfterFunc(delay, func() {
		globalEvents.Fire(event)
	})
}

// Signal is a shortcut for announcing a global event with only a Tag, and no
// Data.
func EventSignal(tag string) {
	globalEvents.Signal(tag)
}

var EventDefaultRetryTime = 3
var EventDefaultRetryDelay = time.Duration(1) * time.Second
...
func EventHandle(prefix string, handler func(data interface{}) error) {
	go func() {
		...
		for event := range globalEvents.Listen(prefix) {
			...
			if err != nil {
				if event.RetryTime < EventDefaultRetryTime {
					log.Warnf(context.Background(), "event:retry:%s,data=%v, failed with err:%v", event.Tag, event.Data, err)
					event.RetryTime += 1
					EventRetry(event, EventDefaultRetryDelay)
				} else {
					log.Errorf(context.Background(), "event:failed:%s,data=%v, failed  with err:%v", event.Tag, event.Data, err)
				}
			} else {
				log.Infof(context.Background(), "event:success:%s,data=%v, ok", event.Tag, event.Data)
			}
		}
	}()
}

func EventRetry(event Event, delay time.Duration) {
	event.RetryTime += 1
	EventFire(event, delay)
}
